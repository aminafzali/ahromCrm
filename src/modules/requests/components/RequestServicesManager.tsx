...
const ServiceCard = ({ service, onSelect, isSelected }: { service: ActualService, onSelect: () => void, isSelected: boolean }) => (
  <button
    type="button"
    onClick={onSelect}
    disabled={isSelected}
    className={`bg-white rounded-lg p-3 text-start border-2 w-full transition-all ${
      isSelected
        ...
        : "border-transparent hover:shadow-lg hover:border-primary"
    }`}
  >
    <div className="flex justify-between items-center">
      <h4 className="font-semibold text-md text-gray-800">{service.name}</h4>
      {isSelected && <DIcon icon="fa-check-circle" classCustom="text-primary" />}
    </div>
...
const RequestServicesManager: React.FC<Props> = ({ control, register, formState }) => {
  ...
  return (
    <div className="card mt-4">
      <div className="card-header">
        <h5 className="card-title mb-0">افزودن خدمات به درخواست</h5>
      </div>
      <div className="card-body">
          ...
          <div className="mb-4 p-3 border rounded-lg">
            ...
            <div className="space-y-3">
              {fields.map((field, index) => (
                <div key={field.id} className="flex items-center gap-3 p-2 bg-gray-50 rounded-md">
                  <div className="flex-grow font-semibold">{getServiceName(field.actualServiceId)}</div>
                  <div className="flex items-center gap-2">
                    <label className="text-sm">تعداد:</label>
                    <input
                      type="number"
                      className={`form-control form-control-sm w-20 ${errors?.[index]?.quantity ? 'is-invalid' : ''}`}
                      {...register(`actualServices.${index}.quantity`, { valueAsNumber: true })}
                    />
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      type="number"
                      className={`form-control form-control-sm w-32 ${errors?.[index]?.price ? 'is-invalid' : ''}`}
                      {...register(`actualServices.${index}.price`, { valueAsNumber: true })}
                    />
                  </div>
        ...
        <div>
          ...
          {isLoading ? <Loading /> : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3" style={{ maxHeight: '400px', overflowY: 'auto' }}>
              {availableServices.map((service) => (
                <ServiceCard
                  key={service.id}
                  service={service}
                  onSelect={() => handleSelectService(service)}
                  isSelected={currentServices.some(s => s.actualServiceId === service.id)}
                />
              ))}
            </div>
          )}
