...
import { InvoiceType } from "@prisma/client"; // <<-- 1. ایمپورت InvoiceType
...
// <<-- 2. اسکیمای Zod با فیلد جدید invoiceNumberName بروزرسانی شد
const invoiceSchema = z.object({
  invoiceNumber: z.number(),
  invoiceNumberName: z.string().min(1, "نام شماره فاکتور الزامی است"),
...
// کامپوننت ItemPicker بدون تغییر باقی می‌ماند...
function ItemPicker({
  isOpen,
  mode,
  items,
...
export default function InvoiceForm({
  ...
  // <<-- 3. State جدید برای نام نمایشی شماره فاکتور
  const [invoiceNumberName, setInvoiceNumberName] = useState(
    defaultValues?.invoiceNumberName || ""
  );
  ...
  // <<-- 4. useEffect اصلاح شده برای گرفتن شماره فاکتور بر اساس نوع
  useEffect(() => {
    if (!defaultValues?.id) {
      const getNextNumber = async () => {
        const data = await fetchNextInvoiceNumber(type);
        setInvoiceNumber(data.invoiceNumber);
        setInvoiceNumberName(data.invoiceNumberName);
      };
      getNextNumber();
    }
  }, [type, defaultValues?.id]); // <<-- به `type` وابسته شد
  ...
  /* ---------- picker open helpers (بدون تغییر) ---------- */
  ...
  const handleSubmit = () => {
    try {
      ...
      // <<-- 5. آبجکت data با فیلدهای جدید بروزرسانی شد
      const data: any = {
        invoiceNumber,
        invoiceNumberName, // <<-- فیلد جدید اضافه شد
    ...
    <div className="space-y-6">
      ...
      <div className="card bg-white shadow-sm border p-4">
        <div className="flex-1 space-y-4">
          ...
          {/* <<-- 6. فیلد شماره فاکتور با نسخه جدید جایگزین شد */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="label">
                <span className="label-text font-medium">شماره فاکتور</span>
              </label>
              <Input
                name="invoiceNumberName"
                value={invoiceNumberName || ""}
                readOnly
