...
export default function SupportInfoDetailPage() {
  const params = useParams() as { id?: string } | null;
  const id = Number(params?.id);
  const router = useRouter();
  const { getById, loading, error, statusCode, remove } = useSupportInfo();
  ...
  const handleDelete = async () => {
    await remove(id);
    router.push("/dashboard/support-info");
  };
  ...
  return (
    <div className="p-2 md:p-4">
      <DetailPageWrapper
        ...
        editUrl={`/dashboard/support-info/${id}/update`}
      />
    </div>
  );
}
