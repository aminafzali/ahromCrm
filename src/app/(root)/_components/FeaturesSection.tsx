  ...
  return (
    <section id="features" className="py-16">
      <div className="text-center mb-10">
        <h2 className="text-3xl font-extrabold">امکانات کلیدی پنل اهرم</h2>
        <p className="text-slate-600 mt-2 max-w-2xl mx-auto">
          ابزارهایی که برای رشد کسب‌وکار خود نیاز دارید — ساده، قدرتمند،
          کاربردی.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {features.map((f) => (
          <div
            key={f.title}
            className="rounded-2xl p-6 bg-white border hover:shadow-xl transition group"
          >
            <div className="flex items-center justify-center w-14 h-14 rounded-lg bg-gradient-to-br from-teal-50 to-white mb-4 mx-auto">
              <DIcon
                icon={f.icon}
                cdi={false}
                classCustom="text-teal-600 text-2xl"
              />
            </div>
            <h3 className="text-lg font-semibold text-center">{f.title}</h3>
            <p className="text-slate-600 mt-3 text-sm text-center">
              {f.description}
            </p>

            <div className="mt-5 text-center">
              <span className="inline-block px-3 py-1 rounded-full text-xs bg-teal-50 text-teal-700">
                بیشتر بدانید
              </span>
            </div>
          </div>
