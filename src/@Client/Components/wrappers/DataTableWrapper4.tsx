...
const KanbanCard = <T,>({
  ...
  const style = {
    ...
    // keep touch-action none so DnDKit touch sensors work - we'll implement manual autoscroll
...
const KanbanView = <T extends { id: number | string }>({
  ...
  // کانتینر کلی که overflow-x دارد — مورد استفاده برای autoscroll
  const containerRef = useRef<HTMLDivElement | null>(null);

  // detect rtl scroll type once
  const rtlTypeRef = useRef<"default" | "negative" | "reverse" | null>(null);
  const detectRtlScrollType = () => {
    if (rtlTypeRef.current) return rtlTypeRef.current;
    try {
      ...
      if (outer.scrollLeft > 0) {
        ...
        outer.scrollLeft = 1;
        if (outer.scrollLeft === 0) rtlTypeRef.current = "negative";
        else rtlTypeRef.current = "reverse";
      }
      document.body.removeChild(outer);
    } catch (e) {
      rtlTypeRef.current = "default";
    }
    return rtlTypeRef.current!;
  };

  // helper: normalize and denormalize scrollLeft
  const getMaxScroll = (el: HTMLElement) =>
    Math.max(0, el.scrollWidth - el.clientWidth);

  const normalizeScrollLeft = (el: HTMLElement) => {
    const rtlType = detectRtlScrollType();
    const max = getMaxScroll(el);
    const raw = el.scrollLeft;
    if (getComputedStyle(el).direction !== "rtl") return raw;
    // rtl
    if (rtlType === "negative") {
      // browser uses negative values
      return -raw;
    } else if (rtlType === "reverse") {
      // scrollLeft goes from max -> 0
      return max - raw;
    } else {
      // default (some browsers positive)
      return raw;
    }
  };

  const denormalizeScrollLeft = (
    el: HTMLElement,
    desiredNormalized: number
  ) => {
    const rtlType = detectRtlScrollType();
    const max = getMaxScroll(el);
    if (getComputedStyle(el).direction !== "rtl") return desiredNormalized;
    if (rtlType === "negative") {
      return -desiredNormalized;
    } else if (rtlType === "reverse") {
      return max - desiredNormalized;
    } else {
      return desiredNormalized;
    }
  };

  // sensors (unchanged) - use touchConfig props
  ...
  // --------------------------
  // Autoscroll logic (mobile)
  // --------------------------
  // minimal-configurable params (اگر خواستی قابل expose از props می‌کنم)
  const edgeThreshold = 60; // px from left/right edge where autoscroll starts
  const maxSpeed = 18; // px per frame at max
  const tickMs = 16; // ms/frame - we use requestAnimationFrame so tickMs is advisory

  const rafRef = useRef<number | null>(null);
  const lastTouchXRef = useRef<number | null>(null);

  // core autoscroll function - reads lastTouchXRef and applies scroll
  const autoscrollTick = () => {
    const container = containerRef.current;
    if (!container || lastTouchXRef.current == null || activeItem == null) {
      rafRef.current = null;
      return;
    }
    ...
    const distLeft = x - leftEdge;
    const distRight = rightEdge - x;

    let speed = 0;
    if (distLeft < edgeThreshold) {
      // scroll left
      const ratio = Math.max(0, (edgeThreshold - distLeft) / edgeThreshold);
      speed = -Math.ceil(ratio * maxSpeed);
    } else if (distRight < edgeThreshold) {
      // scroll right
      const ratio = Math.max(0, (edgeThreshold - distRight) / edgeThreshold);
      speed = Math.ceil(ratio * maxSpeed);
    } else {
      speed = 0;
    }
    ...
    if (speed !== 0) {
      // read normalized current, compute new normalized, then denormalize and apply
      const curr = normalizeScrollLeft(container);
      const max = getMaxScroll(container);
      let next = curr + speed;
      if (next < 0) next = 0;
      if (next > max) next = max;
      const rawNext = denormalizeScrollLeft(container, next);
      // apply via scrollLeft (use scrollTo for smoothness)
      container.scrollLeft = rawNext;
    }
  ...
  // touchmove handler attached to container
  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;
    ...
    // we attach passive: false? we don't preventDefault here; keep passive true to avoid blocking
  ...
  }, [activeItem]); // rebind when activeItem changes
  ...
  return (
    ...
    >
      <div ref={containerRef} className="flex gap-4 overflow-x-auto p-2">
