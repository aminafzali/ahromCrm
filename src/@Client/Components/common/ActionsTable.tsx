...
const ActionsTable: React.FC<TableActionsProps> = ({
  actions,
  row,
  onView,
  onEdit,
  ...
  const activeCustomActions = customActions.filter((ca) =>
    actions.includes(ca.id)
  );
  ...
  return (
    <>
      <div
        className={`
          flex 
          ${direction === "row" ? "flex-row gap-2" : "flex-col gap-2"} 
          ${className}
        `}
      >
        ...
        {actions.includes("edit") && onEdit && (
          <Link className={`text-info flex items-center mx-1 `} href={onEdit}>
            {icons.edit}
            {showLabels && <span className="mr-2">{labels.edit}</span>}
          </Link>
        )}
        ...
        {activeCustomActions.map((action, index) => (
          <Button
            key={index}
            size={size}
            variant="ghost"
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button variant="primary" onClick={confirmationState.onConfirm}>
